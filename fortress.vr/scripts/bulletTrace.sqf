...
{
    {
        ...
        _unit = _x;
        {
            ...
            _positions = missionNameSpace getVariable [format["hyp_var_tracer_projectile_%1", _x], []];
		...
		forEach hyp_var_tracer_activeIndexes;
    }
...
hyp_fnc_traceFire = {
    ...
    hyp_var_tracer_currentIndex = 0;
    hyp_var_tracer_activeIndexes = [];
    _unit setVariable ["hyp_var_tracer_initialized", true];

	if (player == _unit) then 
	{
		[_unit, "FS_Server_EH"] call BIS_fnc_MP;
		hyp_var_tracer_tracedUnits set [count hyp_var_tracer_tracedUnits, _unit];
		publicVariable "hyp_var_tracer_tracedUnits";
	};
};
...
FS_Server_EH = {
	...
	_eventHandle = _this addEventHandler ["Fired", 
	{
...
hyp_fnc_traceFireEvent = {
    private["_this","_params","_initialPos","_unit","_projectile","_color","_lifetime","_interval","_maxDistance","_maxDuration","_startTime","_skippedFrames","_positions","_projIndex","_activeIndexes","_initialVel"];
    ...
    _projIndex     = hyp_var_tracer_currentIndex;  //Get the index to assign to the bullet
    hyp_var_tracer_currentIndex = _projIndex + 1; //Increment index for next bullet
	publicVariable "hyp_var_tracer_currentIndex";
 
    //Initialize final array into which all positions for the current projectile will be stored...
    missionNameSpace setVariable [format["hyp_var_tracer_projectile_%1", _projIndex], _positions];
    publicVariable format["hyp_var_tracer_projectile_%1", _projIndex];
	//...Then update the activeIndexes to indicate that the projectile is active
    _activeIndexes = hyp_var_tracer_activeIndexes;
    _activeIndexes set [count _activeIndexes, _projIndex];
    hyp_var_tracer_activeIndexes = _activeIndexes;
    publicVariable "hyp_var_tracer_activeIndexes";
    ...
    if (_lifetime != -1) then {
        ...
        missionNameSpace setVariable [format["hyp_var_tracer_projectile_%1", _projIndex], nil]; //Delete the projectile's data
		publicVariable format["hyp_var_tracer_projectile_%1", _projIndex];
	};
};
...
//    hyp_var_tracer_currentIndex = nil;
//    hyp_var_tracer_activeIndexes = nil;
